...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/10/27 08:48:39 by smamba            #+#    #+#             */
/*   Updated: 2016/10/27 08:48:39 by smamba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "RainingScene.hpp"

void	RainingScene::WaterUp(t_data *d, Point **buffer) {
	int x, y = 0;
...
void	RainingScene::Rain(t_data * d, bool buffer) {
	int x = 0, y = 0, i = 0;
...
void	RainingScene::upWater( void ) {
	int 		y = 0;
	int 		x;
...
int 	RainingScene::ChoosePath( Point ** oldBuff, Point ** newBuff, int z, int y, int x ) {
	int tmp = z;
	int path = -1;
...
void	RainingScene::rUpWater1(t_data * d, int z, int y, int x) {
...
void	RainingScene::rUpWater2(t_data * d, int z, int y, int x) {
...
void	RainingScene::hillCopy_1to2( t_data * d ) { // Copy NO WATER buffer 2 to buffer 1
	int y = 0;
	int x;
...
void	RainingScene::hillCopy_2to1( t_data * d ) { // Copy NO WATER buffer 1 to buffer 2
	int y = 0;
	int x;
...
void	RainingScene::resetChecked( void ) {
	int y = 0;
	int x;
...
void	RainingScene::initWater( void ) {
...
void	RainingScene::clearBuffer1( void ) {
	int y = 0;
	int x;
	while ( y < TAB_SIZE ) {
		x = 0;
...
void	RainingScene::clearBuffer2( void ) {
	int y = 0;
	int x;
	while ( y < TAB_SIZE ) {
		x = 0;
...
void	RainingScene::print2Dmap( void ) { // print Z from 2dmap used for debugg only
	if ( DEBUGG == true ) {
		int	y = 0;
		while( y < TAB_SIZE ) {
...
RainingScene::RainingScene() : Scenar8FirstCall(true), scenar8Draw('2') {
	this->d = getData();
	this->d->z = -1;
	srand(time(NULL));
}

RainingScene::RainingScene(RainingScene const &rSource ) {
	*this = rSource;
}

RainingScene & RainingScene::operator=( RainingScene const &rSource ) {
	if ( this != &rSource ) {
		this->d = rSource.d;
	}
